...
| `mainPageCloak.js` | Disguises the **non-game** pages' tab title + favicon (default: `Inbox - Gmail`). Reads `mainPageCloak`, `mainCloakTitle`, `mainCloakIcon` from `localStorage`; rewrites `*.png` cloak paths to the matching `cloak-images/favicon/*.ico`. Skips any URL under `/jg/g/`. |
