    ...
    Call() {
        if (-1 === this._timerId) {
            var c = Date.now()
              ...
              , b = this._interval;
            a >= b && this._canRunImmediate ? (this._lastCallTime = c,
...
"use strict";
{
    ...
    window.RuntimeInterface = class g {
        ...
        async _Init(d) {
            ...
            d.baseUrl ? this._baseUrl = d.baseUrl : (k = location.origin,
        ...
        async CreateWorker(d, k, l) {
            ...
            if ("cordova" === this._exportType && this._isFileProtocol)
                return d = await this.CordovaFetchLocalFileAsArrayBuffer(l.isC3MainWorker ? d : this._scriptFolder + d),
                d = new Blob([d],{
        ...
        async _InitDOM(d, k) {
            ...
            for (let[q,t] of r)
                ...
                q === n)
                    try {
                        ...
                        await b(t),
                        "preview" !== this._exportType || l[q] || this._ReportProjectMainScriptError(q, "main script did not run to completion")
                    } catch (x) {
            ...
            alert("Failed to load JavaScript code used in events. Check all your JavaScript code has valid syntax.")) : (d = Object.assign(this._GetCommonRuntimeOptions(d), {
                isInWorker: !1,
                messagePort: k,
                canvas: this._canvas,
        ...
        ["_OnMessageFromRuntime"](d) {
            const k = d.type;
            if ("event" === k)
                return this._OnEventFromRuntime(d);
            if ("result" === k)
                this._OnResultFromRuntime(d);
        ...
        _OnEventFromRuntime(d) {
            ...
            if (d = F.get(k))
                if (d = d.get(l)) {
                    ...
                    console.warn(`[DOM] No handler '${l}' for component '${k}'`);
            else
        ...
        _OnRAFCallback() {
            this._rafId = -1;
            for (const d of this._rafCallbacks)
                d();
            this._RequestAnimationFrame()
        }
        TryPlayMedia(d) {
        ...
        async _MaybeGetCordovaScriptURL(d) {
            return "cordova" === this._exportType && (d.startsWith("file:") || this._isFileProtocol && this.IsRelativeURL(d)) ? (d.startsWith(this._baseUrl) && (d = d.substr(this._baseUrl.length)),
...
"use strict";
{
    ...
    function w(f) {
        do {
            if (f.parentNode && f.hasAttribute("contenteditable"))
                return !0;
            f = f.parentNode
        } while (f);
        return !1
    ...
    c.AddDOMHandlerClass(class extends self.DOMHandler {
        ...
        Attach() {
            ...
            window.addEventListener("pointerrawupdate", g => this._OnPointerRawUpdate(g))) : window.addEventListener("pointermove", g => this._OnPointerEvent("pointermove", g)),
            window.addEventListener("pointerup", g => this._OnPointerEvent("pointerup", g)),
            window.addEventListener("pointercancel", g => this._OnPointerEvent("pointercancel", g))) : (window.addEventListener("mousedown", g => {
                this._HandlePointerDownFocus(g);
        ...
        _OnPointerEvent(f, g) {
            ...
            "mouse" === g.pointerType && (d = "mousemove",
        ...
        _OnPostToDebugger(f) {
            window.c3_postToMessagePort && (f.from = "runtime",
            window.c3_postToMessagePort(f))
        }
        ...
        _OnWrapperMessage(f) {
            ...
            this._OnFullscreenChange()) : "exited-fullscreen" === f ? (c._SetWrapperIsFullscreenFlag(!1),
            this._OnFullscreenChange()) : console.warn("Unknown wrapper message: ", f)
        }
    }
    )
}
        ...
        *audioInstancesByTag(a) {
        ...
        async _Play(a) {
            const b = a.originalUrl
              ...
              , v = a.panning;
            let w = a.off;
            0 < w && !a.trueClock && (this._audioContext.getOutputTimestamp ? (a = this._audioContext.getOutputTimestamp(),
            w = w - a.performanceTime / 1E3 + a.contextTime) : w = w - performance.now() / 1E3 + this._audioContext.currentTime);
            this._lastPlayedTag = p;
            this._AddPendingTag(p);
            try {
                this._lastAudioInstance = await this._GetAudioInstance(b, e, h, p, m),
                v ? (this._lastAudioInstance.SetPannerEnabled(!0),
                this._lastAudioInstance.SetPan(v.x, v.y, v.angle, v.innerAngle, v.outerAngle, v.outerGain),
                v.hasOwnProperty("uid") && this._lastAudioInstance.SetUID(v.uid)) : this._lastAudioInstance.SetPannerEnabled(!1),
        ...
        _SetEffectParam(a) {
            ...
            !a || 0 > b || b >= a.length || (a[b].SetParam(e, h, m, p),
            this._PostUpdatedFxState())
        }
        _RemoveEffects(a) {
            a = a.tag.toLowerCase();
        ...
        _DoPostUpdatedFxState() {
            const a = {};
            for (const [b,e] of this._effects)
                a[b] = e.map(h => h.GetState());
...
"use strict";
{
    ...
    self.C3AudioInstance = class {
        ...
        SetPan(a, b, e, h, m, p) {
            this._isPannerEnabled && (this.SetPanXYA(a, b, e),
...
self.C3Html5AudioInstance = class extends self.C3AudioInstance {
    ...
    SetLooping(c) {
        c = !!c;
        this._isLooping !== c && (this._isLooping = c,
        this.GetAudioElement().loop = c)
    }
    _UpdatePlaybackRate() {
    ...
    SetSuspended(c) {
        c ? this.IsPlaying() ? (this.GetAudioElement().pause(),
...
self.C3WebAudioInstance = class extends self.C3AudioInstance {
    ...
    _OnEnded(c) {
        this._isPaused || this._resumeMe || c.target !== this._activeSource || (this._isStopped = this._hasPlaybackEnded = !0,
        this._instUid = -1,
        this._ReleaseBufferSource(),
        this._audioDomHandler.PostTrigger("ended", this._tag, this._aiId))
    }
    ...
    Seek(c) {
        this._isStopped || this.HasEnded() || (this._isPaused ? this._resumePosition = c : (this.Pause(),
    ...
    SetSuspended(c) {
        c ? this.IsPlaying() ? (this._resumeMe = !0,
...
self.RuntimeInterface.AddDOMHandlerClass(class extends self.DOMHandler {
    constructor(c) {
        super(c, "browser");
        this._exportType = "";
        this.AddRuntimeMessageHandlers([["get-initial-state", a => this._OnGetInitialState(a)], ["ready-for-sw-messages", () => this._OnReadyForSWMessages()], ["alert", a => this._OnAlert(a)], ["close", () => this._OnClose()], ["set-focus", a => this._OnSetFocus(a)], ["vibrate", a => this._OnVibrate(a)], ["lock-orientation", a => this._OnLockOrientation(a)], ["unlock-orientation", () => this._OnUnlockOrientation()], ["navigate", a => this._OnNavigate(a)], ["request-fullscreen", a => this._OnRequestFullscreen(a)], ["exit-fullscreen", () => this._OnExitFullscreen()], ["set-hash", a => this._OnSetHash(a)]]);
...
self.RuntimeInterface.AddDOMHandlerClass(class extends self.DOMHandler {
    constructor(c) {
        super(c, "nwjs");
        this._isNWjs = !1;
        this.AddRuntimeMessageHandlers([["init", () => this.Init()], ["show-open-dlg", a => this._OnShowOpenDlg(a)], ["show-folder-dlg", () => this._OnShowFolderDlg()], ["show-save-dlg", a => this._OnShowSaveDlg(a)], ["set-title", a => this._OnSetTitle(a)], ["set-window-x", a => this._OnSetWindowX(a)], ["set-window-y", a => this._OnSetWindowY(a)], ["set-window-width", a => this._OnSetWindowWidth(a)], ["set-window-height", a => this._OnSetWindowHeight(a)], ["window-cmd", a => this._OnWindowCommand(a)], ["window-attention", a => this._OnWindowRequestAttention(a)], ["set-window-max-size", a => this._OnSetWindowMaxSize(a)], ["set-window-min-size", a => this._OnSetWindowMinSize(a)], ["set-window-resizable", a => this._OnSetWindowResizable(a)], ["set-window-always-top", a => this._OnSetWindowAlwaysOnTop(a)], ["shell-cmd", a => this._OnShellCommand(a)], ["set-clipboard", a => this._OnSetClipboard(a)], ["clear-clipboard", () => this._OnClearClipboard()]])
    }
    GetWindow() {
...
self.RuntimeInterface.AddDOMHandlerClass(class extends self.DOMHandler {
    ...
    async _RequestOrientationPermission() {
        ...
        try {
            ...
            return console.warn("[Touch] Failed to request orientation permission: ", c),
            !1
        }
    }
    async _RequestMotionPermission() {
        ...
        try {
            ...
            return console.warn("[Touch] Failed to request motion permission: ", c),
            !1
        }
    }
}
...
self.RuntimeInterface.AddDOMHandlerClass(class extends self.DOMHandler {
    ...
    _GetSafeAreaInset() {
        ...
        c = [c.getPropertyValue("--temp-sai-top"), c.getPropertyValue("--temp-sai-right"), c.getPropertyValue("--temp-sai-bottom"), c.getPropertyValue("--temp-sai-left")].map(b => {
            b = parseInt(b, 10);
