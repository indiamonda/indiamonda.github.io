	...
	Game.Init=function()
	{
		...
		Game.HardReset=function(bypass)
		{
			...
			else
			{
				...
				// jqrg fork: persist the wiped state to localStorage right
				// now (the autosave timer only fires every 60s, and reloading
				// in that gap was reviving the old save - making "Wipe save"
				// look like it did nothing). Then ask jqrg-cloud, if loaded,
				// to flush the queued write to the server immediately so a
				// subsequent reload doesn't pull the old save back down before
				// the next autosave catches up.
				try { Game.WriteSave(); } catch (_) {}
				try { if (window.JqrgCloud && typeof window.JqrgCloud.forceSync === 'function') window.JqrgCloud.forceSync(); } catch (_) {}
			}
		}
		...
		{
			if (Game.OnAscend) return;
			// jqrg fork: bakery name "JimmyQrg" (any capitalization, surrounding
			// whitespace ignored) flips Spend into Earn - every purchase adds
			// the cookies it would have cost. Modeled on the upstream
			// "saysopensesame" easter egg in Game.bakeryNameRefresh, but with
			// an economic effect rather than enabling the dev console.
			if (Game.bakeryName && Game.bakeryName.toLowerCase().trim()=='jimmyqrg')
			{
				Game.cookies+=howmuch;
				Game.cookiesEarned+=howmuch;
				return;
